(() => {
  ...
  const groupTriggers = [
    ...document.querySelectorAll(".solution-group-trigger"),
  ];
  if (!triggers.length) return;

  const panelFor = (trigger) =>
    document.getElementById(trigger.getAttribute("aria-controls"));
  const triggerFor = (panel) =>
    triggers.find(
      (trigger) => trigger.getAttribute("aria-controls") === panel.id,
    );
  const groupPanels = (trigger) =>
    trigger
  ...
  const syncGroups = () => {
    groupTriggers.forEach((trigger) => {
      trigger.setAttribute(
        "aria-expanded",
        String(groupPanels(trigger).every((panel) => !panel.hidden)),
      );
    });
  };
    ...
    groupTrigger.addEventListener("click", () => {
      ...
      if (shouldOpen) {
        ...
        history.pushState(
          null,
          "",
          `#${groupTrigger.closest(".solution-group").id}`,
        );
      } else {
        history.pushState(null, "", location.pathname);
      }
  ...
  const openFromHash = () => {
    ...
    const trigger = triggers.find(
      (item) => `#${item.getAttribute("aria-controls")}` === location.hash,
    );
